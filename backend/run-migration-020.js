...
async function runMigration() {
  try {
    console.log('🔄 Running migration 020: Create ledger entries for bank transfers...');
    
    const migrationPath = path.join(__dirname, 'src/database/migrations/020_create_ledger_entries_for_bank_transfers.sql');
    const migrationSQL = fs.readFileSync(migrationPath, 'utf8');
    
    await pool.query(migrationSQL);
    
    console.log('✅ Ledger entries created for existing bank transfers!');
