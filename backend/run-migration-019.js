...
async function runMigration() {
  try {
    console.log('🔄 Running migration 019: Create ledger accounts for existing banks...');
    
    const migrationPath = path.join(__dirname, 'src/database/migrations/019_create_ledger_for_existing_banks.sql');
    const migrationSQL = fs.readFileSync(migrationPath, 'utf8');
    
    await pool.query(migrationSQL);
    
    console.log('✅ Ledger accounts created for existing bank accounts!');
