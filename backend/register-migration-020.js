...
async function registerMigration() {
  try {
    console.log('📝 Registering migration 020 in tracking table...');
    
    // Insert migration 020 record
    const result = await pool.query(
      `INSERT INTO migrations (version, name, executed_at) 
       VALUES ($1, $2, NOW()) 
       ON CONFLICT (version) DO NOTHING 
       RETURNING *`,
      [20, 'create_ledger_entries_for_bank_transfers']
    );
    
    if (result.rows.length > 0) {
      console.log('✅ Migration 020 registered successfully');
    } else {
      console.log('ℹ️  Migration 020 was already registered');
    }
