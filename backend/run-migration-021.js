...
async function runMigration() {
  try {
    console.log('🔄 Running migration 021: Fix ledger transaction dates...');
    
    const migrationPath = path.join(__dirname, 'src/database/migrations/021_fix_ledger_transaction_date.sql');
    const migrationSQL = fs.readFileSync(migrationPath, 'utf8');
    
    await pool.query(migrationSQL);
    
    console.log('✅ Ledger transaction dates fixed!');
