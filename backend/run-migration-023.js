...
async function runMigration() {
  try {
    console.log('🔄 Running migration 023: Fix trigger for wd_approved...');
    
    const migrationPath = path.join(__dirname, 'src/database/migrations/023_fix_trigger_for_wd_approved.sql');
    const migrationSQL = fs.readFileSync(migrationPath, 'utf8');
    
    await pool.query(migrationSQL);
    
    console.log('✅ Trigger fixed to handle wd_approved status!');
